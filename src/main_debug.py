#!/usr/bin/env python3
"""
API Detector Inteligente v7.3.4 - Versão Debug
"""
    ...
    try:
        ...
        for i in range(1, len(image_hashes)):
            try:
                ...
                hash_diff = abs(prev_hash - curr_hash) / max(prev_hash, curr_hash, 1) if max(prev_hash, curr_hash) > 0 else 0
                size_diff = abs(prev_size - curr_size) / max(prev_size, curr_size, 1) if max(prev_size, curr_size) > 0 else 0
        ...
        for img in image_hashes:
            if img['timestamp']:
                ...
                if hour not in hourly_analysis:
                    hourly_analysis[hour] = {'screenshots': 0}
...
def home():
    return jsonify({
        ...
        'mode': 'DEBUG - Com logs detalhados',
...
def analyze():
    ...
    try:
        ...
        if zip_result['success'] and zip_result['imageCount'] > 0:
            ...
            print("Iniciando análise de ociosidade")
...
@app.route('/health')
def health():
    return jsonify({'status': 'healthy', 'version': '7.3.4-debug', 'mode': 'debug'})

if __name__ == '__main__':
    print("🚀 Detector Inteligente API v7.3.4 - DEBUG")
    print("📊 Análise com logs detalhados")
